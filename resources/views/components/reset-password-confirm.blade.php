...
<template x-teleport="body">
    <div x-data="{ show: @js($show), title: 'Hapus Data!', route: '#' }" x-init="$watch('show', value => { if (value) { document.body.classList.add('overflow-y-hidden'); } else { document.body.classList.remove('overflow-y-hidden'); }
    })"
        ...
        <div x-show="show"
            ...
            <form :action="route" method="POST" @open-modal.window="route = $event.detail.route">
                @csrf @method('PUT')
                <h2 class="text-lg font-medium" x-text="title"
                    @open-modal.window="title = $event.detail.title"></h2>
                <p class="mt-1 text-sm text-muted-foreground">
                    Password user akan di reset menjadi "password"
                </p>
