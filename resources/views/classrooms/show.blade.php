    ...
    <div class="relative overflow-hidden bg-card text-card-foreground border rounded-lg shadow-sm">
        ...
        <div
            ...
            <div
                ...
                {{ $classroom->tingkat->label() }}
            </div>
            <div class="lg:mt-6 mt-4 lg:ml-4 text-center lg:text-left">
                <h5 class="text-xl lg:text-lg font-bold leading-none tracking-tight">{{ $classroom->nama() }}</h5>
                <small
                    class="block mt-1 text-sm font-medium leading-none text-muted-foreground">{{ $classroom->wali->nama ?? 'Belum pilih wali kelas'}}</small>
            </div>
            <x-button variant="outline" href="{{ route('classrooms.edit', $classroom) }}"
